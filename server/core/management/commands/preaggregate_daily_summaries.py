...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        while current <= max_date:
            invoices_total = (
                Invoice.objects.filter(invoice_date=current).aggregate(
                    total=Sum("total_amount")
                )["total"]
                or 0
            )
            bills_total = (
                Bill.objects.filter(bill_date=current).aggregate(
                    total=Sum("total_amount")
                )["total"]
                or 0
            )
            payments_total = (
                Payment.objects.filter(date=current)
            ...
            self.stdout.write(
                f"Aggregated {current}: invoices={invoices_total}, "
                f"bills={bills_total}, payments={payments_total}"
            )
