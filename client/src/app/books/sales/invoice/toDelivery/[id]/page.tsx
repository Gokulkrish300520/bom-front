...
export default function NewChallanPage() {
  ...
  const [items, setItems] = useState<ChallanItem[]>([{ id: crypto.randomUUID(), itemId: undefined, name: "", qty: 1, rate: 0 }]);
  ...
  useEffect(() => {
    ...
    async function loadInvoiceAndPrefillChallan() {
      try {
        ...
        // map invoice item_details to challan items

        const mappedItems = invoice.item_details.map((itemDetail: any) => ({
        ...
        console.error("Error pre-filling challan from invoice", err);
      }
    }

    loadInvoiceAndPrefillChallan();
  }, []);
      ...
      items.map((it, i) =>
        i === idx
          ? {
              ...
              itemId,
              name: selected?.name || "",
              rate: selected ? Number(selected.sales_selling_price) : 0,
            }
          : it
      )
    );
  ...
  return (
    <div className="min-h-screen p-6 bg-green-50">
      <h1 className="mb-6 text-2xl font-bold text-green-800">New Delivery Challan</h1>
      ...
      <div className="grid grid-cols-1 gap-4 mb-4 md:grid-cols-3">
        <div>
          ...
          <input type="text" value={challanNumber} className="w-full px-3 py-2 border border-green-300 rounded-lg" readOnly />
        </div>
        <div>
                ...
                <tr key={item.id} className="border-b">
                  ...
                  <td className="p-2">
                    <input
                      ...
                      value={item.qty}
                      onChange={e =>
                        setItems(items.map((it, i) => (i === idx ? { ...it, qty: Number(e.target.value) } : it)))
                      }
                    />
                  </td>
                  <td className="p-2">
                    <input
                      ...
                      value={item.rate}
                      onChange={e =>
                        setItems(items.map((it, i) => (i === idx ? { ...it, rate: Number(e.target.value) } : it)))
                      }
                    />
                  </td>
                  <td className="p-2 text-right">{(item.qty * item.rate).toFixed(2)}</td>
      ...
      <div className="flex justify-end gap-3">
        <button type="button" onClick={() => router.push("/books/sales/challans")} className="px-6 py-2 bg-gray-200 rounded-lg">
          Cancel
        </button>
        <button
          type="button"
