...
export default function ImportBillsPage() {
  ...
  const fetchData = async (url?: string) => {
    ...
    try {
      ...
      const fetchUrl = url || `https://web-production-6baf3.up.railway.app/api/nongsts/?${params}`;
      const res = await fetchWithAuth(fetchUrl);
      if (!res.ok) throw new Error("Failed to fetch Non-Gst bills");
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "NonGstBills");
    const buf = XLSX.write(wb, { type: "array", bookType: "xlsx" });
    saveAs(new Blob([buf]), "Non-Gst_bills.xlsx");
  };

  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this entry?")) return;
    try {
      const res = await fetchWithAuth(`https://web-production-6baf3.up.railway.app/api/nongsts/${id}/`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Failed to delete entry");
      toast.success("Non-Gst Data Deleted successfully!");
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `https://web-production-6baf3.up.railway.app/api/nongsts/${selectedBill.id}/`,
        {
          method: "PATCH",
  ...
  return (
    <div className="min-h-screen p-6 bg-white">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <Breadcrumb/>
        <h1 className="text-2xl font-bold text-green-800">Non Gst Bills</h1>
        </div>
        <div className="flex gap-3">
          ...
          <button
            onClick={() => router.push("/books/purchase/nongst/new")}
            ...
            {loading ? (
              ...
              data.map((bill) => (
                <tr key={bill.id} className="border-b hover:bg-green-50">
                  ...
                  <td className="p-2 border border-green-200">{bill.payment_reference_no || "-"}</td>
                  ...
                  <td className="flex gap-2 p-2 border border-green-200">
                    <button
                      onClick={() => router.push(`/books/purchase/nongst/${bill.id}`)}
                      className="px-2 py-1 text-white bg-blue-500 rounded hover:bg-green-700"
                    >
                      View
                    </button>
                    <button
                      onClick={() => router.push(`/books/purchase/nongst/${bill.id}/edit`)}
              ...
              <tr>
                <td colSpan={9} className="p-4 text-center text-green-600">
                  No records found. Click "+ New" to add Non-Gst bills.
                </td>
              </tr>
            )}
          </tbody>
