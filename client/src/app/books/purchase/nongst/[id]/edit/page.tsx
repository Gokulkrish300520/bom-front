...
export default function EditImportBillPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [billRes, vendorRes, dealRes] = await Promise.all([
          fetchWithAuth(`https://web-production-6baf3.up.railway.app/api/nongsts/${params.id}/`),
  ...
  if (!bill) return <p className="p-6 text-red-700">Non-Gst Bill not found.</p>;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetchWithAuth(
        `https://web-production-6baf3.up.railway.app/api/nongsts/${bill.id}/`,
        {
          method: "PUT",
      ...
      toast.success("Gst Bill updated successfully");
      router.push("/books/purchase/non-gst");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen p-6 bg-green-50">
      ...
      <h1 className="mb-6 text-2xl font-bold text-green-900">Edit Non-Gst Bill</h1>
