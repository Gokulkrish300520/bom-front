...
export default function ViewImportBillPage() {
  ...
  const fetchBill = async () => {
    try {
      ...
      console.log("Fetching Non-Gst bill with ID:", id);

      const res = await fetchWithAuth(
        `https://web-production-6baf3.up.railway.app/api/nongsts/${id}/`
      );

      if (!res.ok) {
        console.error("API Error:", res.status, res.statusText);
        throw new Error(`Failed to load Non-Gst bill with ID ${id}`);
      }
      ...
      toast.error("Failed to load Non-Gst bill.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      ...
      {/* Page Title */}
      <h1 className="mb-6 text-3xl font-bold text-green-900">Non-Gst Bill Details</h1>
