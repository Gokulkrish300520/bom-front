...
export default function ImportBillsPage() {
  ...
  const fetchData = async (url?: string) => {
    ...
    try {
      ...
      const fetchUrl = url || `https://web-production-6baf3.up.railway.app/api/importbills/?${params}`;
      const res = await fetchWithAuth(fetchUrl);
      if (!res.ok) throw new Error("Failed to fetch import bills");
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetchWithAuth(`https://web-production-6baf3.up.railway.app/api/importbills/${id}/`, {
        method: "DELETE",
      });
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `https://web-production-6baf3.up.railway.app/api/importbills/${selectedBill.id}/`,
        {
          method: "PATCH",
    ...
    <div className="min-h-screen p-6 bg-white">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => router.push("/books/purchase/import_bills/new")}
              ...
              data.map((bill) => (
                <tr key={bill.id} className="border-b hover:bg-green-50">
                  ...
                  <td className="flex gap-2 p-2 border border-green-200">
                    <button
                      onClick={() => router.push(`/books/purchase/import_bills/${bill.id}`)}
                      className="px-2 py-1 text-white bg-blue-500 rounded hover:bg-green-700"
                    >
                      View
                    </button>
                    <button
                      onClick={() => router.push(`/books/purchase/import_bills/${bill.id}/edit`)}
