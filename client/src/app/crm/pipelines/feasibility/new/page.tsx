...
export default function NewFeasibilityPage() {
  ...
  const [formData, setFormData] = useState<Omit<Feasibility, "id" | "date">>({
    department: "",
    company_name: "",
    contact: "",
    state: "",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    data.push(newItem);
    localStorage.setItem("feasibilityData", JSON.stringify(data));
  ...
  return (
    <div className="min-h-screen p-8 bg-white">
      <h1 className="mb-6 text-2xl font-bold text-green-700">Add New Feasibility Check</h1>
      <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-6 p-6 rounded shadow md:grid-cols-2 bg-green-50">
        ...
        <div>
          ...
          <input type="text" name="source" value={formData.source} onChange={handleChange} required className="w-full p-2 border rounded" />
        </div>
        <div className="md:col-span-2">
          <label className="block font-medium text-green-800">Description <span className="text-red-500">*</span></label>
          <textarea name="description" value={formData.description} onChange={handleChange} required rows={3} className="w-full p-2 border rounded"></textarea>
        </div>
        <div className="md:col-span-2">
        ...
        <div className="flex justify-end col-span-2 gap-4 mt-4">
          <button type="button" onClick={() => router.push('/crm/pipelines/feasibility')} className="px-4 py-2 border rounded bg-gray-200 hover:bg-gray-300">Cancel</button>
