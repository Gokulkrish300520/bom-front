...
// This type must be compatible with the item being created
type PreprocessItem = {
    id: string;
    date: string;
    approval_status: "Pending";
    stage_history?: StageHistory[];
    [key: string]: any; // Allows other properties from the spread
};
...
export default function NegotiationListPage() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      const parsedData: Negotiation[] = JSON.parse(storedData).map((item: any) => ({
        ...item,
        id: item.id || uuidv4(),
  ...
  const handleConfirmConvert = () => {
    ...
    const { quotation_status, closed_reason, followup_datetime, ...rest } = itemToMove;
    
    // MODIFIED: Added stage_history tracking
    const newItem: Partial<PreprocessItem> = {
        ...rest,
        id: uuidv4(),
