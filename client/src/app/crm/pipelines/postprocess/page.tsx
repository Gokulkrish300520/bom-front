...
export default function PostProcessListPage() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      const parsedData: PostProcessItem[] = JSON.parse(storedData).map((item: any) => ({
        ...item,
        id: item.id || uuidv4(),
  ...
  // MODIFIED: Added stage history tracking
  const handleConfirmMove = () => {
    const itemToMove = dialogState.item;
    if (!itemToMove) return;

    const { post_process_status, ...rest } = itemToMove;
                  ...
                  <tr key={item.id} className={`${rowClass} hover:bg-green-50`}>
                    ...
                    <td className="p-2 border">
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button onClick={() => navigate(`/crm/pipelines/postprocess/${item.id}/view`)} className="px-3 py-1 text-xs text-white bg-blue-500 rounded-md hover:bg-blue-600">View</button>
                            {status.text !== 'Completed' && (
                                <button onClick={() => navigate(`/crm/pipelines/postprocess/${item.id}/edit`)} className="px-3 py-1 text-xs text-white bg-yellow-500 rounded-md hover:bg-yellow-600">Edit</button>
                            )}
